...
namespace OnePointRestAPI.Facade
{
    public class PostTicketingRequestFacade : BaseFacade, IPostTicketingRequestFacade
    {
        public dynamic PostTicketingRequest(dynamic value)
        {
            ...
            {  
                ...
                response = (response.Data != null) ? CommonUtils.RetainPropertiesList(response.Data,
                  CommonUtils.SplitStringToList((string)CommonUtils.ResponseFilterConfig.Post_Add_PostTicketingResponse)) : response;
            }
            catch (Exception ex)
            {
        ...
        public dynamic PostTicketingRequestReIssue(dynamic value)
        {
            ...
            {
                response = Common.CommonUtils.RestClient_PostData(
                    string.Join("", (string)CommonUtils.AppConfig.RestEndPoints.CoreBaseEndPoint, "ReIssueQuoteService.ashx"),
                    CommonUtils.JsonSerialize(value), "Post", new Dictionary<string, string>());

                response = (response.Data != null) ? CommonUtils.RetainPropertiesList(response.Data,
               CommonUtils.SplitStringToList((string)CommonUtils.ResponseFilterConfig.Post_Add_PTRReissueResponse)) : response;
