...
{
    public class InvoiceFacade : BaseFacade, IInvoiceFacade
    {
        public dynamic SearchInvoice(dynamic value)
        {
            ...
            try
            {
                ...
                (string)CommonUtils.AppConfig.RestEndPoints.CoreBaseEndPoint, "InvoiceService.ashx?", query));

                response = (response.Data != null) ? CommonUtils.RetainPropertiesList(response.Data,
                   CommonUtils.SplitStringToList((string)CommonUtils.ResponseFilterConfig.Post_Search_InvoiceRequest)) : response;
            }
            catch (Exception ex)
            {
